...
use webrender_traits::{VectorImageResult, VectorImageError, VectorImageDescriptor};
...
struct FakeVectorImageRenderer {
    images: HashMap<ImageKey, VectorImageResult>,
}
...
impl VectorImageRenderer for FakeVectorImageRenderer {
    fn request_vector_image(&mut self, key: ImageKey, _: Arc<VectorImageData>, descriptor: &VectorImageDescriptor) {
                ...
                match descriptor.format {
                    ...
                    _ => {
                        self.images.insert(key,
                            Err(VectorImageError::Other(format!(
        ...
        self.images.insert(key, Ok(RasterizedVectorImage {
            data: texels,
    ...
    fn resolve_vector_image(&mut self, key: ImageKey) -> VectorImageResult {
        self.images.remove(&key).unwrap_or(Err(VectorImageError::InvalidKey))
    }
}
