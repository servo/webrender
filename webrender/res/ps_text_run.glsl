...
VertexInfo write_text_vertex(RectWithSize local_clip_rect,
    ...
    if (transform.is_axis_aligned) {
        ...
        // Transfrom from device space back to local space.
        mat2 local_transform = inverse(device_transform);
    ...
    local_pos = rect_inside_rect(local_rect, local_clip_rect) ?
                    local_transform * (glyph_rect.p0 + glyph_rect.size * aPosition.xy) :
