...
BorderClipDash fetch_border_clip_dash(ivec2 address, int offset) {
    vec4 data[2] = fetch_from_resource_cache_2_direct(address + ivec2(offset, 0));
...
BorderClipDot fetch_border_clip_dot(ivec2 address, int offset) {
    vec4 data = fetch_from_resource_cache_1_direct(address + ivec2(offset, 0));
    ...
    if (cci.segment == 0) {
        ...
        switch (corner.clip_mode) {
            case CLIP_MODE_DASH: {
                ...
                BorderClipDash dash = fetch_border_clip_dash(cci.clip_data_address, 2 + 2 * (cci.segment - 1));
            ...
            case CLIP_MODE_DOT: {
                BorderClipDot cdot = fetch_border_clip_dot(cci.clip_data_address, 2 + (cci.segment - 1));
                vPoint_Tangent0 = vec4(1.0);
                vPoint_Tangent1 = vec4(1.0);
