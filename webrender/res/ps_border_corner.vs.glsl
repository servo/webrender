...
int select_style(int color_select, float style0, float style1) {
    switch (color_select) {
        case SIDE_BOTH:
            return int(style0);
        case SIDE_FIRST:
...
void main(void) {
    ...
    switch (sub_part) {
        ...
        case 1: {
            ...
            style = select_style(prim.user_data.x, border.style.z, border.style.y);
            vec4 adjusted_widths = get_effective_border_widths(border, style);
            vec4 inv_adjusted_widths = border.widths - adjusted_widths;
            set_radii(style,
                      border.radii[0].zw,
        ...
        case 2: {
            ...
            vClipSign = vec2(-1.0, -1.0);
            style = select_style(prim.user_data.x, border.style.w, border.style.z);
            vec4 adjusted_widths = get_effective_border_widths(border, style);
            vec4 inv_adjusted_widths = border.widths - adjusted_widths;
            set_radii(style,
                      border.radii[1].xy,
        ...
        case 3: {
            ...
            vClipSign = vec2(1.0, -1.0);
            style = select_style(prim.user_data.x, border.style.x, border.style.w);
            vec4 adjusted_widths = get_effective_border_widths(border, style);
            vec4 inv_adjusted_widths = border.widths - adjusted_widths;
            set_radii(style,
                      border.radii[1].zw,
