...
void set_radii(float style,
    ...
    vRadii0.zw = get_radii(radii - widths, vec2(-widths));

    switch (int(style)) {
        case BORDER_STYLE_RIDGE:
        case BORDER_STYLE_GROOVE:
            vRadii1.xy = radii - adjusted_widths;
            vRadii1.zw = vec2(-widths);
            break;
        case BORDER_STYLE_DOUBLE:
            vRadii1.xy = get_radii(radii - adjusted_widths, vec2(-widths));
            vRadii1.zw = get_radii(radii - widths + adjusted_widths, vec2(-widths));
            break;
        default:
            vRadii1.xy = vec2(-widths);
            vRadii1.zw = vec2(-widths);
            break;
    }
}
...
void write_color(vec4 color0, vec4 color1, float style, vec2 delta) {
    vec4 modulate = vec4(1.0 + 0.3 * delta.x,
                         1.0 - 0.3 * delta.x,
                         1.0 + 0.3 * delta.y,
                         1.0 - 0.3 * delta.y);
    ...
    switch (int(style)) {
        case BORDER_STYLE_GROOVE:
            vColor00 = vec4(color0.rgb * modulate.y, color0.a);
            vColor01 = vec4(color0.rgb * modulate.x, color0.a);
            vColor10 = vec4(color1.rgb * modulate.w, color1.a);
            vColor11 = vec4(color1.rgb * modulate.z, color1.a);
            break;
        case BORDER_STYLE_RIDGE:
