...
fn body(api: &RenderApi,
    ...
    let geometry_key = api.generate_geometry_key();
    ...
    geometry.push(GeometryItem::Shape(Shape {
        path: path,
        fill: ColorU::new(0xf0, 0xf0, 0x0, 0xff)
    }));
    api.update_geometry(geometry_key, geometry);
    
    let bounds = (250, 100).to(450, 300);
    builder.push_geometry(bounds,
                          None,
                          geometry_key);
