...
impl<'a> FlattenContext<'a> {
    ...
    fn clip_and_scroll_info_with_replacements(&self, info: &mut ClipAndScrollInfo) {
        info.scroll_node_id = self.scroll_frame_id_with_replacement(info.scroll_node_id);
        ...
        for nested_list_info in self.nested_display_list_info.iter().rev() {
            if clip_node_id.is_root_scroll_node() {
                clip_node_id = nested_list_info.1;
            } else {
                break;
            }
        }
    ...
    fn scroll_frame_id_with_replacement(&self, id: ClipId) -> ClipId {
...
impl Frame {
    ...
    fn flatten_stacking_context<'a>(&mut self,
        ...
        let mut clip_id = context.scroll_frame_id_with_replacement(context_scroll_node_id);
    ...
    fn flatten_item<'a, 'b>(&mut self,
        ...
        context.clip_and_scroll_info_with_replacements(&mut clip_and_scroll);
