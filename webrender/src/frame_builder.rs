...
use util::{self, LayerTransformOrOffset, MaxRect, RectHelpers, WorldToLayerTransformOrOffset};
...
pub struct PictureContext<'a> {
    ...
    pub inv_world_transform: Option<WorldToLayerTransformOrOffset>,
}
    ...
    pub fn setup_viewport_offset(
        ...
        if let Some(root_node) = clip_scroll_tree.nodes.get_mut(&root_id) {
            if let NodeType::ReferenceFrame(ref mut info) = root_node.node_type {
                info.resolved_transform = LayerTransformOrOffset::with_offset(
                    LayerVector2D::new(viewport_offset.x, viewport_offset.y)
                );
            }
        }
    }
