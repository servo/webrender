...
impl<'a> LayerRectCalculationAndCullingPass<'a> {
    ...
    fn rebuild_clip_info_stack_if_necessary(&mut self, mut id: ScrollLayerId) -> DeviceIntRect {
        ...
        loop {
            ...
            match node.node_type {
                NodeType::Clip(ref clip) if clip.is_masking() => {
                    if bounding_rect.is_none() {
                        bounding_rect = Some(clip.xf_rect.as_ref().unwrap().bounding_rect);
                    }
