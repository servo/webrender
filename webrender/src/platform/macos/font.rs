...
impl FontContext {
    ...
    fn print_glyph_data(&mut self, cg_context: &mut CGContext, width: usize, height: usize)
    {
        ...
        for i in 0..height {
            ...
            while j < width * 4 {
                let b = data[(current_height+ j) as usize];
                let g = data[(current_height+ j + 1) as usize];
                let r = data[(current_height + j + 2) as usize];
                j += 4;
                print!("({}, {}, {}) ", r, g, b);
            }
            println!("");
    ...
    pub fn rasterize_glyph(&mut self,
        ...
        match self.get_ct_font(font_key, size) {
            Some(ref ct_font) => {
                ...
                for i in 0..rasterized_pixels.len() {
                    rasterized_pixels[i] = 255 - rasterized_pixels[i];
                }
