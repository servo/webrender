...
use core_graphics::base::{kCGBitmapByteOrder32Little, CGFloat};
...
fn get_glyph_metrics(
    ...
    if let Some(transform) = transform {
        bounds = unsafe { CGRectApplyAffineTransform(bounds, *transform) };
        advance = unsafe { CGSizeApplyAffineTransform(advance, *transform) };
    }
...
impl FontContext {
    ...
    pub fn rasterize_glyph(
        ...
        if let Some(transform) = transform {
            unsafe {
                CGContextSetTextMatrix(cg_context.as_concrete_TypeRef(), transform);

                let inv_transform = CGAffineTransformInvert(transform);
                draw_origin = CGPointApplyAffineTransform(draw_origin, inv_transform);
            }
        }
