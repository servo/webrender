...
impl FontContext {
    ...
    fn load_glyph(&self, font: &FontInstance, glyph: &GlyphKey) -> Option<FT_GlyphSlot> {
        ...
        if let Some(FontInstancePlatformOptions { flags, hinting, .. }) = font.platform_options {
    ...
    pub fn is_bitmap_font(&mut self, font: &FontInstance) -> bool {
        ...
        if (face_flags & (FT_FACE_FLAG_FIXED_SIZES as FT_Long)) != 0 {
            if let Some(FontInstancePlatformOptions { flags, .. }) = font.platform_options {
                if (flags & FONT_EMBEDDED_BITMAP) != 0 {
                    return true;
                }
            }
    ...
    fn rasterize_glyph_outline(
        ...
        if let &FontInstance {
            ...
            platform_options: Some(FontInstancePlatformOptions { lcd_filter, .. }),
            ..
        } = font {
    ...
    pub fn rasterize_glyph(
        ...
        // what WR expects.
        let subpixel_bgr = if let Some(FontInstancePlatformOptions { flags, .. }) = font.platform_options {
            (flags & FONT_SUBPIXEL_BGR) != 0
        } else {
            false
        };
