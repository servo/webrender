...
impl FontContext {
    ...
    pub fn rasterize_glyph(
        ...
        let (mut shape, (x_offset, y_offset)) = if bitmaps {
            ...
            (Some(font.transform.invert_scale(y_scale)), font.get_subpx_offset(key))
        };
        if font.flags.contains(FontInstanceFlags::SYNTHETIC_ITALICS) {
            shape = Some(shape.unwrap_or(FontTransform::identity()).synthesize_italics(OBLIQUE_SKEW_FACTOR));
        }
        let transform = shape.map(|shape| dwrote::DWRITE_MATRIX {
            m11: shape.scale_x,
            m12: shape.skew_y,
            m21: shape.skew_x,
            m22: shape.scale_y,
