...
impl FontContext {
    ...
    fn get_dwrite_texture_type(&self, render_mode: FontRenderMode) ->
    ...
    fn get_dwrite_measure_mode(&self, render_mode: FontRenderMode) ->
    ...
    fn get_dwrite_render_mode(&self, render_mode: FontRenderMode) ->
    ...
    let face = self.fonts.get(&font_key).unwrap();
        ...
        let dwrite_render_mode = self.get_dwrite_render_mode(render_mode);
        let dwrite_measure_mode = self.get_dwrite_measure_mode(render_mode);
    ...
    fn get_glyph_dimensions_with_analysis(&self, analysis: dwrote::GlyphRunAnalysis,
    ...
    pub fn get_glyph_dimensions(&self,
        ...
        let texture_type = self.get_dwrite_texture_type(render_mode);
        Some(self.get_glyph_dimensions_with_analysis(analysis, texture_type))
    }

    // DWRITE gives us values in RGB. WR expects it in RGBA. Note, CG returns in BGR
    // TODO: probably should fix that
    fn convert_to_rgba(&self, pixels: &Vec<u8>, render_mode: FontRenderMode) -> Vec<u8> {
        match render_mode {
            FontRenderMode::Mono => {
                let mut rgba_pixels = vec![0 as u8; pixels.len() * 4];
            ...
            FontRenderMode::Alpha => {
                ...
                let mut rgba_pixels = vec![0 as u8; length * 4];
                for i in 0..length {
                    // TODO(vlad): we likely need to do something smarter
                    // This is what skia does
                    let alpha :u8 = ((pixels[i*3+0] as u32 +
                                    pixels[i*3+1] as u32 +
                                    pixels[i*3+2] as u32)
                                    / 3) as u8;
    ...
    pub fn rasterize_glyph(&mut self,
        ...
        self.gdi_gamma_lut.preblend_rgb(&mut pixels, width, height,
