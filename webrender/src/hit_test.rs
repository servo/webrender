...
pub struct HitTestClipScrollNode {
    ...
    regions: Option<Vec<HitTestRegion>>,
...
impl HitTester {
    ...
    fn read_clip_scroll_tree(
        ...
        for (id, node) in &clip_scroll_tree.nodes {
            // XXX: Need to read ClipSources here and generated regions.
    ...
    pub fn is_point_clipped_in_for_node(
        ...
        let regions = match node.regions {
            Some(ref regions) => regions,
            None => {
...
fn get_regions_for_clip_scroll_node(
    ...
    Some(clips.iter().map(|ref source| {
        match source.0 {
