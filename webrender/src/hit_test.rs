        ...
        for &HitTestingRun(ref items, ref clip_and_scroll) in self.runs.iter().rev() {
            ...
            for item in items.iter().rev() {
                ...
                if !item.is_backface_visible {
                    if facing_backwards.is_none() {
                        // Compute facing_backwards the first time we need it.
                        facing_backwards = Some(transform.is_backface_visible());
                    }
                    if facing_backwards.unwrap() {
                        continue;
                    }
                }
