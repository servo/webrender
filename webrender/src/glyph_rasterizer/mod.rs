...
impl FontInstance {
    ...
    pub fn limit_size(&self, x_scale: f64, y_scale: f64) -> f64 {
        // If the scaled size is over the limit, then it will be
        // need to be scaled up from the size limit to the scaled size.
        // However, this should only occur when the font isn't using
        // any features that would tie it to device space, like transforms,
