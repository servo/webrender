...
impl GpuFrameCollection {
    fn draw(&self,
            ...
            y: f32,
            debug_renderer: &mut DebugRenderer) -> Rect<f32> {
        let graph_size = Size2D::new(1024.0, 320.0);
        let bounding_rect = Rect::new(Point2D::new(x, y),
                                      Size2D::new(graph_size.width + 16.0,
                                                  graph_size.height + 16.0));
        let graph_rect = bounding_rect.inflate(-8.0, -8.0);
        ...
        for frame in &self.frames {
            max_time = cmp::max(max_time, frame.total_time);
        }
