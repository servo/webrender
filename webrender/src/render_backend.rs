...
use webrender_traits::channel::{MsgSender, msg_channel, PayloadReceiver, PayloadReceiverHelperMethods};
use webrender_traits::channel::{PayloadSender, PayloadSenderHelperMethods};
use webrender_traits::{ApiMsg, RendererId, BlobImageRenderer, BuiltDisplayList, DeviceIntPoint};
...
impl RenderBackendThread {
    pub fn new(thread_name: String) -> Result<Self, ::std::io::Error> {
        let (api_tx, api_rx) = try! { msg_channel() };
        let (init_tx, init_rx) = channel();

        try!{ thread::Builder::new().name(thread_name.clone()).spawn(move || {
