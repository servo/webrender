    ...
    pub fn update_transform(&mut self, state: &TransformUpdateState) {
        ...
        let (local_transform, accumulated_scroll_offset) = match self.node_type {
            ...
            NodeType::StickyFrame(sticky_frame_info) => {
                ...
                let scrolled_parent_combined_clip =
...
pub struct ReferenceFrameInfo {
    ...
    /// The original, untransformed and relative to the parent reference frame, origin of this
    /// reference frame. This is already rolled into the `transform' property, but we also store it
    /// here to properly transform the viewport for sticky positioning.
    pub original_origin: LayerVector2D,
}
