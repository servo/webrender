...
pub enum NodeType {
    ...
    Clip {
        ...
        /// The clip scroll node that provides the transform for this clip node.
        positioning_node_index: ClipScrollNodeIndex,
    },
...
impl NodeType {
    fn is_reference_frame(&self) -> bool {
        match *self {
            NodeType::Positioning { kind: PositioningNodeType::ReferenceFrame(_), .. } => true,
            _ => false,
        }
    }
}
...
impl ClipScrollNode {
    ...
    pub fn new_scroll_frame(
        ...
        let node_type = NodeType::Positioning {
            kind: PositioningNodeType::ScrollFrame(ScrollFrameInfo::new(
                *frame_rect,
                scroll_sensitivity,
                LayoutSize::new(
    ...
    pub fn new_reference_frame(
        ...
        Self::new(
            ...
            NodeType::Positioning {
                kind: PositioningNodeType::ReferenceFrame(info),
            },
        )
    }
    ...
    pub fn new_sticky_frame(
        ...
        let node_type = NodeType::Positioning {
            kind: PositioningNodeType::StickyFrame(sticky_frame_info),
        };
        Self::new(pipeline_id, Some(parent_index), node_type)
    }
    ...
    pub fn apply_old_scrolling_state(&mut self, old_scroll_info: &ScrollFrameInfo) {
        match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ScrollFrame(ref mut scrolling), .. } => {
    ...
    pub fn set_scroll_origin(&mut self, origin: &LayoutPoint, clamp: ScrollClamping) -> bool {
        ...
        let scrolling = match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ScrollFrame(ref mut scrolling), .. } => scrolling,
            _ => {
                warn!("Tried to scroll a non-scroll node.");
                return false;
            }
    ...
    pub fn update(
        ...
        match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ReferenceFrame(info), .. } if !info.invertible => {
                self.mark_uninvertible();
                return;
            }
    ...
    pub fn update_transform(
        ...
        if let NodeType::Positioning { kind: PositioningNodeType::StickyFrame(ref mut info), .. } = self.node_type {
    ...
    pub fn update_transform_for_reference_frame(
        ...
        let info = match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ReferenceFrame(ref mut info), .. } => info,
    ...
    fn calculate_sticky_offset(
        ...
        let info = match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::StickyFrame(ref info), .. } => info,
    ...
    pub fn prepare_state_for_children(&self, state: &mut TransformUpdateState) {
        ...
        match self.node_type {
            NodeType::Positioning { ref kind } => {
                match *kind {
                    PositioningNodeType::StickyFrame(ref info) => {
                    ...
                    PositioningNodeType::ScrollFrame(ref scrolling) => {
                        state.parent_accumulated_scroll_offset =
    ...
    pub fn scrollable_size(&self) -> LayoutSize {
        match self.node_type {
           NodeType::Positioning { kind: PositioningNodeType::ScrollFrame(state), .. } => state.scrollable_size,
            _ => LayoutSize::zero(),
        }
    }
    ...
    pub fn scroll_offset(&self) -> LayoutVector2D {
        match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ScrollFrame(ref scrolling), .. } => scrolling.offset,
    ...
    pub fn matches_external_id(&self, external_id: ExternalScrollId) -> bool {
        match self.node_type {
            NodeType::Positioning { kind: PositioningNodeType::ScrollFrame(info), .. } if info.external_id == Some(external_id) => true,
            _ => false,
        }
    }
}
