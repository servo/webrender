...
pub struct GradientData {
    pub colors: [PackedColor; 2 * GRADIENT_DATA_RESOLUTION],
}
...
impl GradientData {
    ...
    fn fill_colors(&mut self, start_idx: usize, end_idx: usize, start_color: &ColorF, end_color: &ColorF) -> usize {
        ...
        let inv_steps = 2.0 / (end_idx - start_idx) as f32;
        ...
        let mut cur_packed_color = PackedColor::from_color(&cur_color).to_bgra();

        // Walk the ramp writing start and end colors for each entry.
        while cur_idx < end_idx {
            self.colors[cur_idx] = cur_packed_color;

            cur_color.r += step_r;
            cur_color.g += step_g;
            cur_color.b += step_b;
            cur_color.a += step_a;
            cur_packed_color = PackedColor::from_color(&cur_color).to_bgra();
            self.colors[cur_idx + 1] = cur_packed_color;
    ...
    fn get_index(offset: f32) -> usize {
        2 * (offset * GRADIENT_DATA_RESOLUTION as f32).round() as usize
    }
    ...
    fn build(&mut self, src_stops: &[GradientStop], reverse_stops: bool) {
        ...
        self.fill_colors(cur_idx, 2 * GRADIENT_DATA_RESOLUTION, &cur_color, &cur_color);
    }
}
        ...
        let metadata = match container {
            ...
            PrimitiveContainer::AngleGradient(gradient_cpu, gradient_gpu) => {
                ...
                let metadata = PrimitiveMetadata {
                    ...
                    gpu_data_count: GRADIENT_DATA_RESOLUTION as i32,
                    render_task: None,
                    clip_task: None,
                };
