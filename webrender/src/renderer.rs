...
impl Renderer {
            ...
            for ExternalCaptureImage { short_path, external, descriptor } in renderer.external_images {
                ...
                let tid = match native_map.entry(plain_ext.data) {
                    ...
                    Entry::Vacant(e) => {
                        ...
                        // This texture will be deallocated by delete_external_texture which
                        // doesn't update the allocated memory counter.
                        self.device.deallocated_gpu_memory(t.0.size_in_bytes());
    ...
    fn log_allocation_failure(&self, error: AllocError) {
        ...
        error!("Excessive GPU alloc: {:?}, budget: {}", report, budget);
    }
