...
impl<Src, Dst> MatrixHelpers<Src, Dst> for TypedTransform3D<f32, Src, Dst> {
    ...
    fn is_simple_2d_translation(&self) -> bool {
        ...
        self.m43 < NEARLY_ZERO && self.m44 < NEARLY_ZERO
    }
}
...
pub enum TransformOrOffset<Src, D> {
    ...
    Transform {
        transform: TypedTransform3D<f32, Src, D>,
        inverse: Option<TypedTransform3D<f32, D, Src>>,
        is_2d: bool,
    },
...
impl<Src, D> TransformOrOffset<Src, D> {
    pub fn identity() -> Self {
        TransformOrOffset::Offset(TypedVector2D::zero())
    }

    pub fn with_offset(offset: TypedVector2D<f32, Src>) -> Self {
        TransformOrOffset::Offset(offset)
    }

    #[inline(always)]
    pub fn new_transform(transform: TypedTransform3D<f32, Src, D>) -> Self {
        if transform.is_simple_2d_translation() {
            return TransformOrOffset::Offset(TypedVector2D::new(transform.m41, transform.m42));
        }
        let inverse = transform.inverse();
        let is_2d = transform.is_2d();
        TransformOrOffset::Transform { transform, inverse, is_2d}
    }
    ...
    pub fn to_transform(&self) -> TypedTransform3D<f32, Src, D> {
        match *self {
            ...
            TransformOrOffset::Transform { transform, .. } => transform
        }
    }
    ...
    pub fn pre_mul<NewSrc>(
        &self,
        other: &TransformOrOffset<NewSrc, Src>
    ) -> TransformOrOffset<NewSrc, D> {
        match (self, other) {
            (&TransformOrOffset::Offset(ref offset), &TransformOrOffset::Offset(ref other_offset)) => {
                ...
                TransformOrOffset::Offset((offset + *other_offset))
            }
            _ => {
    ...
    pub fn pre_translate(&self, other_offset: &TypedVector2D<f32, Src>) -> Self {
        match self {
            ...
            &TransformOrOffset::Transform { transform, .. } =>
                TransformOrOffset::new_transform(transform.pre_translate(other_offset.to_3d()))
        }
    }
    ...
    pub fn has_perspective_component(&self) -> bool {
        match *self {
            TransformOrOffset::Offset(..) => false,
            TransformOrOffset::Transform { transform, .. } => transform.has_perspective_component(),
        }
    }
    ...
    pub fn is_backface_visible(&self) -> bool {
        match *self {
            TransformOrOffset::Offset(..) => true,
            TransformOrOffset::Transform { transform, .. } => transform.is_backface_visible(),
        }
    }

    #[inline(always)]
    pub fn transform_point2d(&self, point: &TypedPoint2D<f32, Src>) -> TypedPoint2D<f32, D> {
        match *self {
            TransformOrOffset::Offset(offset) => {
                let new_point = *point + offset;
                TypedPoint2D::from_untyped(&new_point.to_untyped())
            }
            TransformOrOffset::Transform { transform, .. } => transform.transform_point2d(point),
        }
    }

    #[inline(always)]
    pub fn transform_point3d(&self, point: &TypedPoint3D<f32, Src>) -> TypedPoint3D<f32, D> {
        match *self {
            TransformOrOffset::Offset(offset) =>
                TypedPoint3D::new(point.x + offset.x, point.y + offset.y, point.z),
            TransformOrOffset::Transform { transform, .. } => transform.transform_point3d(point),
        }
    }

    #[inline(always)]
    pub fn transform_rect(&self, rect: &TypedRect<f32, Src>) -> TypedRect<f32, D> {
        match *self {
            TransformOrOffset::Offset(offset) =>
    ...
    pub fn unapply(&self, rect: &TypedRect<f32, D>) -> Option<TypedRect<f32, Src>> {
        match *self {
            TransformOrOffset::Offset(offset) =>
                Some(TypedRect::from_untyped(&rect.to_untyped().translate(&-offset.to_untyped()))),
            TransformOrOffset::Transform { inverse: Some(inverse), is_2d: true, .. }  =>
            ...
            TransformOrOffset::Transform { inverse: None, .. }  => None,
        }
    }
    ...
    pub fn post_translate(&self, new_offset: TypedVector2D<f32, D>) -> Self {
        match *self {
            TransformOrOffset::Offset(offset) => {
                let offset = offset.to_untyped() + new_offset.to_untyped();
                TransformOrOffset::Offset(TypedVector2D::from_untyped(&offset))
            }
            TransformOrOffset::Transform { transform, .. } => {
    ...
    #[inline(always)]
    pub fn inverse(&self) -> Option<TransformOrOffset<D, Src>> {
        match *self {
            TransformOrOffset::Offset(offset) =>
                Some(TransformOrOffset::Offset(TypedVector2D::new(-offset.x, -offset.y))),
            TransformOrOffset::Transform { transform, inverse: Some(inverse), is_2d, } =>
                Some(TransformOrOffset::Transform {
                    transform: inverse,
    ...
    pub fn update(&self, transform: TypedTransform3D<f32, Src, D>) -> Option<Self> {
        if transform.is_simple_2d_translation() {
...
pub type LayoutTransformOrOffset = TransformOrOffset<LayoutPixel, LayoutPixel>;
pub type LayerTransformOrOffset = TransformOrOffset<LayerPixel, LayerPixel>;
pub type LayerToWorldTransformOrOffset = TransformOrOffset<LayerPixel, WorldPixel>;
pub type WorldToLayerTransformOrOffset = TransformOrOffset<WorldPixel, LayerPixel>;
