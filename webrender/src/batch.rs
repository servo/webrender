...
impl AlphaBatchBuilder {
    ...
    fn add_prim_to_batch(
        ...
        let legacy_blend_mode = if !prim_metadata.opacity.is_opaque ||
        ...
        match prim_metadata.prim_kind {
            ...
            PrimitiveKind::Border => {
                ...
                let corner_key = BatchKey::new(corner_kind, legacy_blend_mode, no_textures);
                let edge_kind = BatchKind::Transformable(
                    transform_kind,
                    TransformBatchKind::BorderEdge,
                );
                let edge_key = BatchKey::new(edge_kind, legacy_blend_mode, no_textures);
                    ...
                    Some(PictureSurface::RenderTask(cache_task_id)) => {
                        ...
                        match picture.kind {
                            PictureKind::TextShadow { .. } => {
                                ...
                                let key = BatchKey::new(kind, legacy_blend_mode, textures);
                                let batch = self.batch_list.get_suitable_batch(key, &task_relative_bounding_rect);
            ...
            PrimitiveKind::AlignedGradient => {
                ...
                let key = BatchKey::new(kind, legacy_blend_mode, no_textures);
                let batch = self.batch_list.get_suitable_batch(key, &task_relative_bounding_rect);
