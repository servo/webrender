...
pub fn tiles(
    ...
    let x_count = {
        let result = f32::ceil((visible_rect.max_x() - prim_rect.origin.x) / layer_tile_size.width) as i32 - t0.x;
    ...
    let y_count = {
        let result = f32::ceil((visible_rect.max_y() - prim_rect.origin.y) / layer_tile_size.height) as i32 - t0.y;
