...
pub enum RenderTaskLocation {
    ...
    /// phase, we invoke `RenderTargetList::alloc()` and store the resulting
    /// location in the first member. That location identifies the render target
    /// and the offset of the allocated region within that target.
