...
use font_renderer::{FontRenderer, GlyphCache, GlyphRequest};
...
impl ResourceCache {
    pub fn new(texture_cache: TextureCache,
        ...
        ResourceCache {
            ...
            font_renderer: FontRenderer::new(workers),
    ...
    pub fn request_glyphs(&mut self,
        ...
        self.font_renderer.request_glyphs(
    ...
    pub fn get_glyph_dimensions(&mut self, glyph_key: &GlyphKey) -> Option<GlyphDimensions> {
        match self.cached_glyph_dimensions.entry(glyph_key.clone()) {
            ...
            Vacant(entry) => {
                *entry.insert(self.font_renderer.get_glyph_dimensions(glyph_key))
            }
        }
    }
    ...
    pub fn block_until_all_resources_added(&mut self,
        ...
        self.font_renderer.resolve_glyphs(
            self.current_frame_id,
            &mut self.cached_glyphs,
