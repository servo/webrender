        ...
        for cmd in &self.cmds {
            match *cmd {
                PrimitiveRunCmd::PushStackingContext(stacking_context_index) => {
                    ...
                    match stacking_context.composite_kind {
                        CompositeKind::None => {}
                        CompositeKind::Simple(..) | CompositeKind::Complex(..) => {
                            let stacking_context_rect =
                                stacking_context.xf_rect.as_ref().unwrap().bounding_rect;
                            let location =
                                RenderTaskLocation::Dynamic(None, stacking_context_rect.size);
                ...
                PrimitiveRunCmd::PopStackingContext => {
                    ...
                    match stacking_context.composite_kind {
                        ...
                        CompositeKind::Complex(info) => {
                            let stacking_context_rect =
                                stacking_context.xf_rect.as_ref().unwrap().bounding_rect;
                            let readback_task = RenderTask::new_readback(stacking_context_index,
