...
impl ClipScrollTree {
    ...
    fn update_node_transform(&mut self, layer_id: ClipId, state: &TransformUpdateState) {
        ...
        let (state, node_children) = {
            ...
            let state = match node.node_type {
                NodeType::ReferenceFrame(_, ref original_origin) => TransformUpdateState {
                    ...
                    parent_scroll_offset: LayerVector2D::zero(),
                    parent_accumulated_scroll_offset: LayerVector2D::zero(),
                    nearest_scrolling_ancestor_offset: state.nearest_scrolling_ancestor_offset,
                    nearest_scrolling_ancestor_viewport:
                        state.nearest_scrolling_ancestor_viewport.translate(original_origin),
                },
                NodeType::Clip(..) | NodeType::StickyFrame(..) => TransformUpdateState {
                    ...
                    parent_combined_viewport_rect: node.combined_local_viewport_rect,
                    parent_scroll_offset: LayerVector2D::zero(),
                    parent_accumulated_scroll_offset: state.parent_accumulated_scroll_offset,
                    nearest_scrolling_ancestor_offset: state.nearest_scrolling_ancestor_offset,
    ...
    fn print_node(&self, id: &ClipId, pt: &mut PrintTree) {
        ...
        match node.node_type {
            ...
            NodeType::ReferenceFrame(ref transform, _) => {
            ...
            NodeType::StickyFrame(constrant_rect) => {
                pt.new_level(format!("StickyFrame"));
                pt.add_item(format!("constraint rect: {:?}", constrant_rect));
            }
        }
