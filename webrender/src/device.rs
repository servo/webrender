...
fn get_gl_format_bgra() -> gl::GLuint {
    GL_FORMAT_BGRA
}
        ...
        let (gl_format, bpp, data) = match self.textures.get(&texture_id).unwrap().format {
            ImageFormat::A8 => {
                if cfg!(any(target_arch="arm", target_arch="aarch64")) {
                    ...
                    (get_gl_format_bgra(), 4, expanded_data.as_slice())
                } else {
                    (GL_FORMAT_A, 1, data)
                }
            }
            ImageFormat::RGB8 => (gl::RGB, 3, data),
            ImageFormat::RGBA8 => (get_gl_format_bgra(), 4, data),
...
fn gl_texture_formats_for_image_format(format: ImageFormat) -> (gl::GLint, gl::GLuint) {
    match format {
        ImageFormat::A8 => {
            if cfg!(any(target_arch="arm", target_arch="aarch64")) {
                (get_gl_format_bgra() as gl::GLint, get_gl_format_bgra())
            } else {
        ...
        ImageFormat::RGBA8 => {
            match gl::GlType::default() {
                gl::GlType::Gl =>  {
                    (gl::RGBA as gl::GLint, get_gl_format_bgra())
                }
                gl::GlType::Gles => {
                    (get_gl_format_bgra() as gl::GLint, get_gl_format_bgra())
                }
