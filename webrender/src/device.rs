...
impl<'a> Device<'a> {
    ...
    pub fn create_program(
        ...
        if self.cached_programs.is_some() {
            if let Some(binary) = self.cached_programs.as_ref().unwrap().get(&sources)
            {
        ...
        if self.cached_programs.is_some() {
            if !self.cached_programs.as_ref().unwrap().contains(&sources) {
                let (buffer, format) = self.gl.get_program_binary(pid);
                if buffer.len() > 0 {
                  self.cached_programs.as_mut().unwrap().insert(sources, ProgramBinary::new(buffer, format));
                  //let mut programs = self.cached_programs.unwrap();
                  //programs.insert(sources, ProgramBinary::new(buffer, format));
                }
            }
        }
