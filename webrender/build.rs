...
fn get_shader_source(shader_name: &str, shaders: &HashMap<String, String>) -> String {
    let shader_file_path = Path::new(&shaders[shader_name]);
    let mut shader_source_file =
        File::open(shader_file_path).expect(&format!("Unable to open shader file: {:?}", shader_file_path));
    ...
    for line in source.lines() {
        if line.starts_with(SHADER_IMPORT) {
            ...
            for import in imports {
                let include = get_shader_source(import, shaders);
...
fn build_shader_strings(
    ...
    let shared_source = get_shader_source(base_filename, shaders);
...
fn generate_shaders(out_dir: &str, shaders: HashMap<String, String>) {
    let file = File::open("res/shaders.ron").expect("Unable to open shaders.ron");
    let shader_configs: Vec<Shader> = de::from_reader(file).expect("Unable to deserialize shaders.ron");
