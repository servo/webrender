...
fn validate_shaders() {
    ...
    for (filename, file_source) in SHADERS.iter() {
        ...
        let mut shader_source = format!("#version 300 es\n
            // Base shader: {}\n
            #define WR_MAX_VERTEX_TEXTURE_WIDTH {}\n",
            base_filename, webrender::renderer::MAX_VERTEX_TEXTURE_WIDTH);

        if is_vert {
            shader_source.push_str("#define WR_VERTEX_SHADER\n");
        }
        else {
            shader_source.push_str("#define WR_FRAGMENT_SHADER\n");
        }
        if is_clip {
            shader_source.push_str("#define WR_FEATURE_TRANSFORM\n");
        }
