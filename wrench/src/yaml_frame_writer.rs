...
fn write_sc(parent: &mut Table, sc: &StackingContext, scene: &Scene, filter_iter: AuxIter<FilterOp>) {
    enum_node(parent, "scroll-policy", sc.scroll_policy);

    matrix4d_node(parent, "transform", &scene.properties.resolve_layout_transform(&sc.transform));
        ...
        match filter {
            ...
            FilterOp::Opacity(x) => {
                filters.push(Yaml::String(format!("opacity({})",
                                                  scene.properties.resolve_float(&x, 1.0))))
            }
        ...
        loop {
            ...
            match *base.item() {
                ...
                PushStackingContext(item) => {
                    ...
                    write_sc(&mut v, &item.stacking_context, scene, filters);
