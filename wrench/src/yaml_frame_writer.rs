        ...
        loop {
            ...
            match *base.item() {
                ...
                StickyFrame(item) => {
                    ...
                    let horizontal = vec![
                        Yaml::Real(item.horizontal_constraints.min_offset.to_string()),
                        Yaml::Real(item.horizontal_constraints.max_offset.to_string()),
                    ];
                    let vertical = vec![
                        Yaml::Real(item.vertical_constraints.min_offset.to_string()),
                        Yaml::Real(item.vertical_constraints.max_offset.to_string()),
                    ];

                    yaml_node(&mut v, "horizontal-constraints", Yaml::Array(horizontal));
                    yaml_node(&mut v, "vertical-constraints", Yaml::Array(vertical));
                }
