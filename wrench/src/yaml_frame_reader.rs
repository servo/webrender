...
impl YamlFrameReader {
    ...
    fn to_sticky_offset_constraints(&mut self, item: &Yaml) -> StickyOffsetConstraints {
        match *item {
            Yaml::Array(ref array) => StickyOffsetConstraints::new(
                array[0].as_f32().unwrap_or(0.0),
                array[1].as_f32().unwrap_or(0.0),
            ),
            _ => StickyOffsetConstraints::new(0.0, 0.0),
        }
    }
    ...
    pub fn handle_sticky_frame(
        ...
        let id = dl.define_sticky_frame(
            ...
            self.to_sticky_offset_constraints(&yaml["vertical-constraints"]),
            self.to_sticky_offset_constraints(&yaml["horizontal-constraints"]),
        );
