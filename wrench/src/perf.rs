...
impl<'a> PerfHarness<'a> {
    ...
    fn render_yaml(&mut self, filename: &Path) -> TestProfile {
        ...
        // frame profiles. Then take the median.
        ...
        cpu_frame_profiles.sort_by(|a, b| {
            assert!(a.draw_calls == b.draw_calls);
            a.composite_time_ns.cmp(&b.composite_time_ns)
        });
        gpu_frame_profiles.sort_by(|a, b| {
            a.paint_time_ns.cmp(&b.paint_time_ns)
        });
        ...
        let composite_time = cpu_samples.iter()
                                        .fold(0, |acc, ref x| acc + x.composite_time_ns);
        let paint_time = gpu_samples.iter()
                                    .fold(0, |acc, ref x| acc + x.paint_time_ns);
        ...
        TestProfile {
            ...
            draw_calls: cpu_frame_profiles[0].draw_calls,
        }
    }
}
