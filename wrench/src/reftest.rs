        ...
        for line in file.lines() {
            ...
            match items[0] {
                ...
                x => {
                    ...
                    let mut offset = 0;
                    if x.starts_with("fuzzy(") {
                        let (_, args) = parse_function(x);
                        max = args[0].parse().unwrap();
                        count = args[1].parse().unwrap();
                        offset = 1;
                    }
                    let kind = parse_operator(items[offset]).expect("unexpected match operator");
                    let test = dir.join(items[offset + 1]);
                    let reference = dir.join(items[offset + 2]);
...
fn parse_operator(s: &str) -> Option<ReftestOp> {
    match s {
