...
fn render<'a>(
    ...
    let input_path = subargs.value_of("INPUT").map(|s| PathBuf::from(s)).unwrap();
    ...
    let mut thing = if input_path.as_path().is_dir() {
        ...
        let extension = input_path
            .extension()
            .unwrap_or_else(|| panic!("Tried to render with an unknown file type."))
            .to_str()
            .unwrap_or_else(|| panic!("Tried to render with an unknown file type."));
        ...
        if "yaml" == extension {
            ...
            panic!("Tried to render with an unknown file type.")
        }
    };
