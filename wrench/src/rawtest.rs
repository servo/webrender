...
impl<'a> RawtestHarness<'a> {
    ...
    fn test_hit_testing(&mut self) {
        ...
        let mut space_and_clip = SpaceAndClipInfo::root_scroll(self.wrench.root_pipeline_id);
        ...
        space_and_clip.clip_id = builder.define_clip(
            &space_and_clip,
            rect,
        ...
        space_and_clip.clip_id = ClipId::ClipChain(clip_chain_id);
        let mut info = LayoutPrimitiveInfo::new(rect);
        info.tag = Some((0, 5));
        builder.push_rect(&info, &space_and_clip, ColorF::new(1.0, 1.0, 1.0, 1.0));
